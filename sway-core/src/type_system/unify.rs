...
enum UnifierKind {
    ...
    UnifyBoth,
    /// Indicates that we want to unify the left argument to match the right
    /// argument.
    OnlyUnifyLeft,
    /// Indicates that we want to unify the right argument to match the left
    /// argument.
    OnlyUnifyRight,
    /// Indicates that we are unifying for Abstract Data Types.
    UnifyForADTs,
}
...
impl<'a> Unifier<'a> {
    ...
    /// Takes `self` and returns a new [Unifier] with the contents of self and
    /// a new [UnifierKind::OnlyUnifyLeft].
    pub(super) fn left(self) -> Unifier<'a> {
        Unifier {
            kind: UnifierKind::OnlyUnifyLeft,
            ..self
        }
    }

    pub(super) fn for_adts(self) -> Unifier<'a> {
        Unifier {
            kind: UnifierKind::UnifyForADTs,
            ..self
        }
    }
        ...
        match (
            ...
            (Numeric, e @ UnsignedInteger(_)) => {
                ...
                match self
                    .type_engine
                    .slab
                    .replace(received, &Numeric, e, self.type_engine)
                {
                    None => (vec![], vec![]),
            ...
            (r @ UnsignedInteger(_), Numeric) => {
                if let UnifierKind::OnlyUnifyLeft = self.kind {
                    return (vec![], vec![]);
                }
                match self
                    .type_engine
                    .slab
                    .replace(expected, &Numeric, r, self.type_engine)
                {
                    None => (vec![], vec![]),
                    Some(_) => self.unify(received, expected, span, help_text),
                }
            ...
            (Unknown, e) => {
                ...
                match self
                    .type_engine
                    .slab
                    .replace(received, &Unknown, e, self.type_engine)
                {
                    None => (vec![], vec![]),
                    Some(_) => self.unify(received, expected, span, help_text),
                }
            ...
            (r, Unknown) => {
                ...
                match self
                    .type_engine
                    .slab
                    .replace(expected, &Unknown, r, self.type_engine)
                {
                    None => (vec![], vec![]),
                    Some(_) => self.unify(received, expected, span, help_text),
                }
            ...
            (
                ...
                match self.kind {
                    ...
                    UnifierKind::OnlyUnifyLeft => {
                        self.type_engine.insert_unified_type(expected, received);
                    }
                    UnifierKind::OnlyUnifyRight => {
                        self.type_engine.insert_unified_type(received, expected);
                    }
                    UnifierKind::UnifyForADTs => {
                        self.type_engine.insert_unified_type(received, expected);
                        self.type_engine.insert_unified_type(expected, received);
                    }
                }
            ...
            (r @ UnknownGeneric { .. }, e @ UnknownGeneric { .. })
                if self.kind == UnifierKind::UnifyForADTs =>
            {
            ...
            (r @ UnknownGeneric { .. }, e) if self.kind != UnifierKind::OnlyUnifyRight => {
                ...
                match self
                    .type_engine
                    .slab
                    .replace(received, &r, e, self.type_engine)
                {
                    None => (vec![], vec![]),
                    Some(_) => self.unify(received, expected, span, help_text),
                }
            }
            (r, e @ UnknownGeneric { .. }) if self.kind == UnifierKind::UnifyForADTs => {
                self.type_engine.insert_unified_type(received, expected);
                match self
                    .type_engine
                    .slab
                    .replace(expected, &e, r, self.type_engine)
                {
                    None => (vec![], vec![]),
                    Some(_) => self.unify(received, expected, span, help_text),
                }
            ...
            (r, e @ UnknownGeneric { .. })
                if self.kind != UnifierKind::OnlyUnifyLeft
                    && self.kind != UnifierKind::UnifyForADTs =>
            {
                self.type_engine.insert_unified_type(received, expected);
                match self
                    .type_engine
        ...
        WithTypeEngine<'a, T>: fmt::Display,
    {
        ...
        (r, e)
        // if self.arguments_are_flipped || self.kind == UnifierKind::UnifyForADTs {
        //     (e, r)
        // } else {
        //     (r, e)
        // }
